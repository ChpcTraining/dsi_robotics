...
int currentServoAngle[14]; // Array to track the current angle for each servo
  ...
  for (byte index=0; index <=13; index++) {
    if (arduinoPins[index].type != input) {
      ...
      if (arduinoPins[index].type == servomotor) {
        ...
        currentServoAngle[index] = 90; // Initialize servo to 90 degrees (or any default)
        servo(index, currentServoAngle[index]);
      } else {
...
void updateActuator(byte pinNumber) {
  if (arduinoPins[pinNumber].type == digital) {
    ...
    int newAngle = arduinoPins[pinNumber].state;
    if (newAngle == 10) {
      // Increase by 10 degrees
      currentServoAngle[pinNumber] = constrain(currentServoAngle[pinNumber] + 10, 0, 180);
    } else if (newAngle == -10) {
      // Decrease by 10 degrees
      currentServoAngle[pinNumber] = constrain(currentServoAngle[pinNumber] - 10, 0, 180);
    }
    servo(pinNumber, currentServoAngle[pinNumber]);
  }
}
